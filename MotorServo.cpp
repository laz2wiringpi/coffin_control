...
int  MotorServo::check(){
	...
	if (_last_direction == DIRECTION_OFF){
		...
		if (cur_vol == 0) {
			...
			float volcalc = ((abs(_last_speed) * VOL_FACTOR) * (target_vol - cur_vol)) * VOL_FACTOR;
			_cur_lastvolavgcnt = (volcalc / _cur_volavgcnt);
			newspeed = _last_speed + volcalc;

#ifdef DEBUG  

			Serial.print(" volcalc  ");
			Serial.print(volcalc, 4);
			Serial.print(" _cur_volavgcnt ");
			Serial.println(_cur_volavgcnt, DEC);
			Serial.print(" _cur_lastvolavgcnt ");
			Serial.println(_cur_lastvolavgcnt, 4);
#endif
			/*
...
int  MotorServo::Settarget_pos(int atarget_pos){
	...
	if (_target_pos > POT_MAX){
		_target_pos = POT_MAX;
	}
	else if (_target_pos < POT_MIN )
	{
		_target_pos = POT_MIN;
	}
	else
